...
Node.prototype.request = function request(target, type, args, callback) {
  ...
  var self = this,
      ...
      packet = Buffer.concat([new Buffer('W.'), bencode.encode(msg)]);

  try {
    this.socket.send(packet, 0, packet.length, target.port, target.address);
  } catch(e) {
    console.log("Range failed, target port not within bounds: ", target.port);
    return;
  }
...
Node.prototype.respond = function respond(target, msg, args) {
  ...
  var response = {
      ...
      packet = Buffer.concat([new Buffer('W.'), bencode.encode(response)]);

  try {
    this.socket.send(packet, 0, packet.length, target.port, target.address);
  } catch (e) {
    console.log("Dgram error ", e);
...
Node.prototype.onmessage = function onmessage(packet, rinfo) {
  try {
    var preamble = packet.slice(0,2);
    // 'W.' === 0x57, 0x2e 
    if(preamble[0]===0x57 && preamble[1]===0x2e) {
      //console.log('wish');
    } else {
      console.log('someone else...', bencode.decode(packet));
    }
  ...
  // in some case the decode succeeds, but msg is undefined
